...
export default function LoginPage() {
  ...
  return (
    <main className="absolute inset-0 z-[100] flex min-h-screen overflow-hidden bg-surface">
      ...
      >
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1544148103-0773bf10d330?q=80&w=1600&auto=format&fit=crop')" }} />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/60" />
        <div className="absolute inset-0 p-12 flex flex-col justify-between">
           <Link href="/" className="inline-flex max-w-max items-center gap-2 rounded-full bg-black/40 px-4 py-2 text-sm font-semibold tracking-widest text-white backdrop-blur hover:bg-black/60 transition">
             <ArrowLeft className="h-4 w-4" /> Home
           </Link>
           <div>
             <h2 className="font-serif text-5xl text-white">Your table is waiting.</h2>
             <p className="mt-4 max-w-md text-lg text-white/80">Log in to discover exclusive curations and high-signal recommendations from circles you trust.</p>
           </div>
        </div>
      </motion.div>
      ...
      >
        <div className="w-full max-w-md">
          ...
          <Link href="/" className="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-muted transition hover:text-foreground lg:hidden">
          ...
          <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold uppercase tracking-wider text-muted ml-2" htmlFor="email">Email</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-semibold uppercase tracking-wider text-muted ml-2" htmlFor="password">Password</label>
              <div className="group relative flex items-center rounded-2xl border border-border/80 bg-white px-4 transition-all focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20 shadow-sm">
